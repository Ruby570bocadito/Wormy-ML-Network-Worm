...
EXPANDED_LAB_TARGETS = [
    ...
    # ── METASPLOITABLE ──
    {
        ...
        "ip": "127.0.0.1",
        "port": 8084,
        "open_ports": [21, 22, 23, 80, 445, 3306, 5432, 5900, 6667, 8180],
        "os_guess": "Linux",
        ...
        for user, pwd in target.get("creds", []):
            try:
                ...
                req = urllib.request.Request(
                    f"http://{target['ip']}:{target['port']}/api/overview",
...
def test_docker_api(target):
    try:
        ...
        with urllib.request.urlopen(
            f"http://{target['ip']}:{target['port']}/version", timeout=5
        ) as resp:
            data = json.loads(resp.read())
...
def test_kubernetes(target):
    try:
        ...
        try:
            with urllib.request.urlopen(
                f"https://{target['ip']}:{target['port']}/api", context=ctx, timeout=3
            ) as resp:
                if resp.status == 200:
...
def test_gitlab(target):
    try:
        ...
        try:
            with urllib.request.urlopen(
                f"http://{target['ip']}:{target['port']}/api/v4/version", timeout=3
            ) as resp:
                data = json.loads(resp.read())
        ...
        try:
            ...
            req = urllib.request.Request(
                f"http://{target['ip']}:{target['port']}/api/v4/session",
                data=data,
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                ...
                if "private_token" in resp_data:
                    return (
                        True,
                        f"GitLab authenticated (token: {resp_data['private_token'][:8]}...)",
                    )
        except Exception:
            pass
...
def test_couchdb(target):
    try:
        ...
        with urllib.request.urlopen(f"http://{target['ip']}:{target['port']}/", timeout=5) as resp:
            ...
            return (
                True,
                f"CouchDB {data.get('version','?')} | {data.get('vendor',{}).get('name','?')}",
            )
    except Exception as e:
        return False, str(e)
...
def test_influxdb(target):
    try:
        ...
        with urllib.request.urlopen(
            f"http://{target['ip']}:{target['port']}/ping", timeout=5
        ) as resp:
    ...
    try:
        ...
        modbus_req = bytes(
            [
                ...
                0x06,
                0x01,
                0x03,  # Transaction ID, Protocol, Length, Unit ID, Function
                0x00,
                0x00,
                0x00,
                0x01,  # Start Address, Quantity
            ]
        )
...
def main():
    console.print(
        "[bold green]═══ WORMY EXPANDED DOCKER LAB — FULL EXPLOITATION TEST ═══[/bold green]\n"
    )
